...
$post_summary    = urlencode( strip_tags( get_the_excerpt() ) );
$twitter_account = urlencode( preg_replace( "/https?\:\/\/twitter\.com\//", "", get_option( 'twitter-url' ) ) );
$tweet_text      = utf8_encode( urlencode( sprintf( __( 'Check out %1$s on %2$s', 'zingdesign' ), get_the_title(), get_bloginfo( 'name' ) ) ) );
...
$share_twitter = 'https://twitter.com/intent/tweet?url=' . $post_link . '&text=' . $tweet_text . '&via=' . $twitter_account;
...
<div class="post-share">
	...
	<div class="row">
		<div class="medium-4 columns">
			<a href="<?php echo ( $share_facebook ); ?>" class="button facebook" target="_blank"
			   title="Share on Facebook"><i class="fa fa-facebook fa-right-margin"></i> Facebook</a>
		</div>
		<div class="medium-4 columns">
			<a href="<?php echo ( $share_twitter ); ?>" class="button twitter" target="_blank"
			   title="Tweet about this post"><i class="fa fa-twitter fa-right-margin"></i> Twitter</a>
		</div>
		<div class="medium-4 columns">
			<a href="<?php echo ( $share_google ); ?>" class="button google-plus" target="_blank"
